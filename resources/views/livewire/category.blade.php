<div>
    {{-- Create Modal --}}
    <div x-data="{ isOpen: $wire.entangle('createCategory')}">
        <div
        x-show="isOpen"
                ...
                <div class="relative p-4 bg-gray-200 rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div class="mx-auto max-w-xs">
                            <label for="example1" class="mb-1 block text-sm font-medium text-gray-900 dark:text-white" >
                                Upload file
                            </label>
                            <input id="example1" type="file" class="mt-2 block w-full text-sm file:mr-4 file:rounded-md file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60 text-gray-900 dark:text-white" wire:model="photo" name="photo"/>
                        </div>
                    </div>
    ...
    {{-- Update Modal --}}
    <div x-data="{ isOpen: $wire.entangle('updateCategory')}">
        <div
            x-show="isOpen"
            class=" flex fixed inset-0 z-40 min-h-full overflow-y-auto overflow-x-hidden transition items-center justify-center"
            >
                    ...
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label for="description"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category
                                Description</label>
                            <textarea id="description" rows="5" wire:model="updateDescription"
                                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Write post content here"></textarea>
                            @error('description')
                                <p class="text-red-600">{{ $message }}</p>
                        ...
                        <div class="mx-auto max-w-xs">
                            ...
                            <input id="example1" type="file" class="mt-2 block w-full text-sm file:mr-4 file:rounded-md file:border-0 file:bg-teal-500 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-white hover:file:bg-teal-700 focus:outline-none disabled:pointer-events-none disabled:opacity-60 text-gray-900 dark:text-white" wire:model="updatePhoto" name="updatePhoto"/>
                        </div>
                    </div>
                    <button type="button" wire:click="categoryUpdate"
                        class="text-white inline-flex items-center bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                        <svg class="mr-1 -ml-1 w-6 h-6" fill="currentColor" viewBox="0 0 20 20"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd"
    ...
    {{-- Delete Modal --}}
    <div x-data="{ isOpen: $wire.entangle('deleteCategory') }">
                    ...
                    <div class="space-y-2 p-2">
                        <div class="p-4 space-y-2 text-center dark:text-white">
                            <h2 class="text-xl font-bold tracking-tight" id="page-action.heading">
                                Delete {{ $deleteCategoryName }} Category
                            </h2>
                        ...
                        <div class="p-4">
                            <div class="flex justify-between w-full">
                                ...
                                <form class="flex items-center max-w-md justify-end">
                                    ...
                                    wire:model="search" />
                                    <button
                                    wire:click.prevent="searchCategory"
                            ...
                            <div class="overflow-x-auto mt-4">
                                <div class="p-6 bg-gray-100 ">
                                    <div class="text-center text-2xl font-bold mb-6">Categories</div>
                                        @if (!$categories)
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                                @foreach($categories as $category)
                                                    <div class="bg-white rounded-xl shadow-md hover:shadow-2xl overflow-hidden transform transition-all hover:translate-y-2">
                                                        <img src="{{ $category->image ? env('APP_URL')."/storage/".$category->image : 'https://p16-va-tiktok.ibyteimg.com/obj/musically-maliva-obj/df2319bb31ac87ac015128373b630b11.png' }}" alt="Category Image" class="w-full h-48 object-cover">
                                                        <div class="p-4">
                                                            <h3 class="text-lg font-bold truncate">{{ $category->name }}</h3>
                                                            <p class="mt-2 text-gray-600 text-sm truncate " >{{ $category->description }}</p>
                                                            <div class="mt-4 flex justify-start items-center gap-3">
                                                                <a  x-data="{ loading: false }" wire:click="openUpdateModal({{ $category->id }})" href="#" class="bg-yellow-500 text-white px-3 py-1 rounded-md hover:bg-yellow-700 transition duration-300">
                                                                    Edit
                                                                </a>
                                                                <a wire:click="openDeleteModal({{ $category->id }})" href="#" class="bg-red-500 text-white px-3 py-1  rounded hover:bg-red-700 transition duration-300">Delete</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endforeach
